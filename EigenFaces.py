...
class FaceRecognition():
    ...
    def __init__(self, dataSetPath: str):
        ...
        self.error_image = cv2.imread("404.jpg", cv2.IMREAD_GRAYSCALE)
    ...
    def recognize_face(self,image,weights_thresh):
        img = cv2.resize(np.copy(image),(self.dataset_images[0].shape[1],self.dataset_images[0].shape[0]), interpolation = cv2.INTER_AREA)
        zero_mean_image = img.flatten() - self.mean_vector
        ...
        if min_distance == None:
            return cv2.resize(self.error_image,self.dataset_images[0].shape), None
        else:
